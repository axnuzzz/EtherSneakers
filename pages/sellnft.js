...
const SellNft = () => {
  ...
  const resellSneaker = async () => {
    try {
      ...
      const txt = await contract.resellSneaker(tokenId, price, {
        value: listingPrice,
      });
      await txt.wait();
  ...
  return (
    <div className="relative ">
      <Head>
        <title> Sell Token || EtherSneakers </title>
        ...
        <section className="mx-w-[1024px] mx-auto my-10 flex flex-col items-center justify-center">
          <div className="w-[30%] md:w-[60%] sm:w-full sm:p-3 ssm:w-full ">
            ...
            <button
              ...
              onClick={resellSneaker}
              disabled={btnLoading}
            >
              {btnLoading ? "Re Selling Sneaker" : "Sell Sneaker"}
            </button>
          </div>
        </section>
        <Footer />
