...
const SneakerDetails = () => {
  ...
  const buyNft = async (n) => {
    try {
      ...
      await tx.wait();
      toast.success(`Bought the Sneaker🎉`);
  ...
  const reSellNFT = async (sneaker) => {
    router.push(
      `sellnft?tokenId=${sneaker.tokenId}&tokenURI=${sneaker.tokenURI}`
    );
  };
  ...
  return (
    <div>
      <Head>
        <title>{sneaker.name} || EtherSneakers</title>
      ...
      <div className="relative overflow-hidden">
        <section className="grid grid-cols-2 max-w-[1240px] mx-auto my-2 gap-4 font-body sm:grid-cols-1 p-5">
          ...
          <div className="">
            ...
            <div>
              <p>Price</p>
              <h2>{sneaker.price} ETH</h2>
            </div>
            <div>
              <p>Owner </p>
              <h2 className="my-0 ssm:text-sm text-transparent font-bold bg-clip-text bg-gradient-to-r from-sky-500 to-blue-600">
                {truncateEthAddress(sneaker.owner)}
              </h2>
            </div>

            {sneaker.seller.startsWith("0x0") ? null : (
              <div>
                <p>Seller </p>
                <h2 className="my-0 ssm:text-sm text-transparent font-bold bg-clip-text bg-gradient-to-r from-sky-500 to-blue-600">
                  {truncateEthAddress(sneaker.seller)}
                </h2>
              </div>
            )}
            ...
            <button
              ...
              onClick={() => {
                addr === sneaker.owner.toLocaleLowerCase()
                  ? reSellNFT(sneaker)
                  : buyNft(sneaker);
              }}
            >
