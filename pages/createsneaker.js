...
const Create = () => {
  ...
  async function handleFileChange(e) {
    ...
    setSneakerDetails({ ...sneakerDetails, image: uploadedFile });
  ...
  const handleUpload = async () => {
    const { name, description, price, image } = sneakerDetails;
    if (name === "") {
      toast.error("Please provide name for the Sneaker");
    } else if (description === "") {
      toast.error("Please provide description for the Sneaker");
    } else if (price === "") {
  ...
  const uploadToArweave = async (url) => {
    const { name, description } = sneakerDetails;
  ...
  const mintNFT = async (tokenURI) => {
    try {
      ...
      const price = ethers.utils.parseUnits(sneakerDetails.price, "ether");

      let listingPrice = await contract.getListingPrice();
      listingPrice = listingPrice.toString();

      let transaction = await contract.createSneaker(tokenURI, price, {
        value: listingPrice,
      });
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen ">
        <h3 className="text-4xl font-body text-center">
          Oops! Before Publishing your Sneaker Please Add Some Funds.🪙
        </h3>
        <FundWallet />
      </div>
    );
  ...
  return (
    <div className="font-body">
      <Head>
        <title>Create a Sneaker Sale</title>
        <link rel="shortcut icon" href="logo.png" />
      </Head>

      <Header />

      <h1 className="text-center">EtherSneakers Create Sneaker</h1>
        ...
        <section className="max-w-[1024px] my-20 mx-auto grid grid-cols-2  gap-10 font-body  overflow-hidden top-7 md:gap-10 medium md:px-5 sm:grid-cols-1 sm:h-full relative ">
          ...
          >
            <input
              ...
              ref={dataRef}
            />
            {sneakerDetails.image ? (
              <div className="w-full h-full flex justify-center items-center">
                <img
                  src={window.URL.createObjectURL(sneakerDetails.image)}
                  alt="image"
                  ref={sneakerDetails.image}
          ...
          <div className="w-full flex flex-col font-body gap-5">
            <div className="flex flex-col">
              <label className="text-2xl my-1 font-semibold ">
                Name of the Sneaker
              </label>
              <input
               ...
               placeholder:text-slate-400 outline-none border-none  bg-[#272D37]/60 placeholder:font-body font-body"
                value={sneakerDetails.name}
                onChange={(e) =>
                  setSneakerDetails({ ...sneakerDetails, name: e.target.value })
                }
              />
            </div>
            ...
            <div className="flex flex-col">
              ...
              <textarea
                ...
                value={sneakerDetails.description}
                onChange={(e) =>
                  setSneakerDetails({
                    ...sneakerDetails,
